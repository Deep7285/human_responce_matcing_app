...
if coachee_file and mentor_file:
    if st.button(" Run Matching Algorithm", type="primary", use_container_width=True):
        ...
        loading_msg = "🔄 Running Advanced Semantic Context based Analysis (this may take some time)..." if is_advanced else "🔄 Running Standard Word Match Analysis..."
            ...
            if not is_advanced:
                ...
                for idx, c_row in coachee_df.iterrows():
                    ...
                    for i, (m_idx, m_row) in enumerate(candidates.iterrows()):
                        ...
                        # 1. Split Hard and Soft Scores
                        total_hard = (sc_spec * w_spec) + (sc_deg * w_deg)
                        total_soft = (s_prof[i] * w_prof) + (s_pers[i] * w_pers) + (s_iit[i] * w_iit) + (s_back[i] * w_back)
                        
                        # 2. HOLISTIC SYNERGY BONUS
                        # Calculate the maximum possible soft score based on the sidebar sliders
                        max_possible_soft = w_prof + w_pers + w_iit + w_back
